...
export default {
    ...
    mounted() {
        ...
        axiosx({
            method: "GET",
            url: `detail/offspring/${this.$route.params.id}`,
            message: "正在查询子代"
        }).then(res => {
            if (res.data.code === 200) {
                //装填
                const list = []
                res.data.data.forEach((generation, index) => {
                    if (index > 0 && generation.length > 0) {
                        generation.forEach(pigeon => {
                            list.push({
                                ...pigeon,
                                key: list.length + 1,
                                generation: index
